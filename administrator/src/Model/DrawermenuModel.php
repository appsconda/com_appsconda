 ...
 * Appsconda model
 */
class DrawermenuModel extends AdminModel
{
	/**
	...
	{
		// Get the form
		$form = $this->loadForm('com_appsconda.drawermenu', 'drawermenu', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_appsconda.edit.drawermenu.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (@$table->ordering === '') {
				$db = Factory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__appsconda_drawermenus');
				$max = $db->loadResult();
