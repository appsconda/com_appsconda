 ...
 * Appsconda model
 */
class PushModel extends AdminModel
{
	/**
	...
	{
		// Get the form
		$form = $this->loadForm('com_appsconda.push', 'push', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_appsconda.edit.push.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (@$table->ordering === '') {
				$db = Factory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__appsconda_pushs');
				$max = $db->loadResult();
