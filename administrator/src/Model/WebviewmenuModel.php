 ...
 * Appsconda model
 */
class WebviewmenuModel extends AdminModel
{
	/**
	...
	{
		// Get the form
		$form = $this->loadForm('com_appsconda.webviewmenu', 'webviewmenu', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_appsconda.edit.webviewmenu.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (@$table->ordering === '') {
				$db = Factory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__appsconda_webviewmenus');
				$max = $db->loadResult();
