 ...
 * Appsconda model
 */
class MobileconfigsModel extends ListModel
{
	/**
    ...
    public function __construct($config = array())
    {
        if (empty($config['filter_fields']))
        {
            $config['filter_fields'] = array(
				'id', 'a.id',
				'myprofileicon', 'a.myprofileicon',
				'myprofilecolor', 'a.myprofilecolor',
				'myprofilelabel', 'a.myprofilelabel',
				'myprofileshow', 'a.myprofileshow',
	...
	protected function getListQuery()
	{
		...
		$query->select('a.id, a.myprofileicon, a.myprofilecolor');
		$query->select('a.myprofilelabel, a.myprofileshow, a.state');
		$query->select('a.ordering');

		$query->from('`#__appsconda_mobileconfigs` AS a');

		$query->select('f.name AS `created_by`');
		$query->leftJoin($this->_db->qn('#__users') . ' AS `f` ON f.id = a.created_by');
