...
namespace Joomla\Component\Appsconda\Administrator\View\Compilation;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		$title = Text::_('COM_APPSCONDA_TITLE_COMPILATION');
		$icon = 'fa fa-file-alt';
		...
		{
			ToolbarHelper::apply('compilation.apply', 'JTOOLBAR_APPLY');
			//ToolbarHelper::save('compilation.save', 'JTOOLBAR_SAVE');
			ToolbarHelper::custom('compilation.compile', 'icon-check', 'icon-class-over', 'JTOOLBAR_COMPILE', false);
		}

		if (!$checkedOut && ($canDo['core.create']))
		{
			//ToolbarHelper::custom('compilation.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
		}

		// If an existing item, can save to a copy.
		if (!$isNew && $canDo['core.create'])
		{
			//ToolbarHelper::custom('compilation.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
		}

		if (empty($this->item->id))
		{
			//ToolbarHelper::cancel('compilation.cancel', 'JTOOLBAR_CANCEL');
		}
		else
		{
			//ToolbarHelper::cancel('compilation.cancel', 'JTOOLBAR_CLOSE');
		}
	}
}
