...
namespace Joomla\Component\Appsconda\Administrator\View\Compilations;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null)
	{
		...
		AppscondaHelper::addSubmenu('compilations');
	...
	protected function addToolbar()
	{
		...
		$title = Text::_('COM_APPSCONDA_TITLE_COMPILATIONS');
		$icon = 'fa fa-file-alt';
		...
		{
		    ToolbarHelper::addNew('compilation.add','JTOOLBAR_NEW');
	    }

        if ($canDo['core.edit'] && isset($this->items[0]))
		{
		    ToolbarHelper::editList('compilation.edit','JTOOLBAR_EDIT');
	    }
		...
		if ($canDo['core.edit.state'])
		{
            if (isset($this->items[0]->state))
			{
			    ...
			    ToolbarHelper::custom('compilations.publish', 'publish.png', 'publish_f2.png','JTOOLBAR_PUBLISH', true);
			    ToolbarHelper::custom('compilations.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
            }
			else if (isset($this->items[0]))
			{
                //If this component does not use state then show a direct delete button as we can not trash
                ToolbarHelper::deleteList('', 'compilations.delete','JTOOLBAR_DELETE');
            }

            if (isset($this->items[0]->state))
			{
			    ToolbarHelper::divider();
			    ToolbarHelper::archiveList('compilations.archive','JTOOLBAR_ARCHIVE');
            }
            if (isset($this->items[0]->checked_out))
			{
            	ToolbarHelper::custom('compilations.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
            }
		}
        ...
        if (isset($this->items[0]->state))
		{
			...
			{
			    ToolbarHelper::deleteList('', 'compilations.delete','JTOOLBAR_EMPTY_TRASH');
			    ToolbarHelper::divider();
		    }
			else if ($canDo['core.edit.state'])
			{
			    ToolbarHelper::trash('compilations.trash','JTOOLBAR_TRASH');
			    ToolbarHelper::divider();
		    }
        }
