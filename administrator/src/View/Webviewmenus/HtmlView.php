...
namespace Joomla\Component\Appsconda\Administrator\View\Webviewmenus;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null)
	{
		...
		AppscondaHelper::addSubmenu('webviewmenus');
	...
	protected function addToolbar()
	{
		...
		$title = Text::_('COM_APPSCONDA_TITLE_WEBVIEWMENUS');
		$icon = 'fa fa-file-alt';
		...
		{
		    ToolbarHelper::addNew('webviewmenu.add','JTOOLBAR_NEW');
	    }

	    if ($canDo['core.edit'] && isset($this->items[0]))
		{
		    ToolbarHelper::editList('webviewmenu.edit','JTOOLBAR_EDIT');
	    }
		...
		if ($canDo['core.edit.state'])
		{
            if (isset($this->items[0]->state))
			{
			    ...
			    ToolbarHelper::custom('webviewmenus.publish', 'publish.png', 'publish_f2.png','JTOOLBAR_PUBLISH', true);
			    ToolbarHelper::custom('webviewmenus.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
            }
			else if (isset($this->items[0]))
			{
                //If this component does not use state then show a direct delete button as we can not trash
                ToolbarHelper::deleteList('', 'webviewmenus.delete','JTOOLBAR_DELETE');
            }

            if (isset($this->items[0]->state))
			{
			    ToolbarHelper::divider();
			    ToolbarHelper::archiveList('webviewmenus.archive','JTOOLBAR_ARCHIVE');
            }
            if (isset($this->items[0]->checked_out))
			{
            	ToolbarHelper::custom('webviewmenus.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
            }
		}
        ...
        if (isset($this->items[0]->state))
		{
			...
			{
			    ToolbarHelper::deleteList('', 'webviewmenus.delete','JTOOLBAR_EMPTY_TRASH');
			    ToolbarHelper::divider();
		    }
			else if ($canDo['core.edit.state'])
			{
			    ToolbarHelper::trash('webviewmenus.trash','JTOOLBAR_TRASH');
			    ToolbarHelper::divider();
		    }
        }
	...
	protected function loadTemplateHeader() : void
	{
		...
		if ($saveOrder)
		{
			...
			$this->saveOrderingUrl = 'index.php?option=com_appsconda&task=webviewmenus.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
		}
