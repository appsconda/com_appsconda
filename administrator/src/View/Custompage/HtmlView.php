...
namespace Joomla\Component\Appsconda\Administrator\View\Custompage;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		$title = Text::_('COM_APPSCONDA_TITLE_CUSTOMPAGE');
		$icon = 'fa fa-file-alt';
		...
		{
			ToolbarHelper::apply('custompage.apply', 'JTOOLBAR_APPLY');
			ToolbarHelper::save('custompage.save', 'JTOOLBAR_SAVE');
		}

		if (!$checkedOut && ($canDo['core.create']))
		{
			ToolbarHelper::custom('custompage.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
		}

		// If an existing item, can save to a copy.
		if (!$isNew && $canDo['core.create'])
		{
			ToolbarHelper::custom('custompage.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
		}

		if (empty($this->item->id))
		{
			ToolbarHelper::cancel('custompage.cancel', 'JTOOLBAR_CANCEL');
		}
		else
		{
			ToolbarHelper::cancel('custompage.cancel', 'JTOOLBAR_CLOSE');
		}
	}
}
