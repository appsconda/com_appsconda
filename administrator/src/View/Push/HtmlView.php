...
namespace Joomla\Component\Appsconda\Administrator\View\Push;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		$title = Text::_('COM_APPSCONDA_TITLE_PUSH');
		$icon = 'fa fa-file-alt';
		...
		{
			ToolbarHelper::apply('push.apply', 'JTOOLBAR_APPLY');
			ToolbarHelper::save('push.save', 'JTOOLBAR_SAVE');
			ToolbarHelper::custom('push.saveandsend', 'icon-check', 'icon-class-over', 'JTOOLBAR_SAVE_AND_SEND', false);

		}

		if (!$checkedOut && ($canDo['core.create']))
		{
			ToolbarHelper::custom('push.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
		}

		// If an existing item, can save to a copy.
		if (!$isNew && $canDo['core.create'])
		{
			ToolbarHelper::custom('push.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
		}

		if (empty($this->item->id))
		{
			ToolbarHelper::cancel('push.cancel', 'JTOOLBAR_CANCEL');
		}
		else
		{
			ToolbarHelper::cancel('push.cancel', 'JTOOLBAR_CLOSE');
		}
	}
}
