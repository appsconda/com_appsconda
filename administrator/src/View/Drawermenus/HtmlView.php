...
namespace Joomla\Component\Appsconda\Administrator\View\Drawermenus;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null)
	{
		...
		AppscondaHelper::addSubmenu('drawermenus');
	...
	protected function addToolbar()
	{
		...
		$title = Text::_('COM_APPSCONDA_TITLE_DRAWERMENUS');
		$icon = 'fa fa-file-alt';
		...
		{
		    ToolbarHelper::addNew('drawermenu.add','JTOOLBAR_NEW');
	    }

	    if ($canDo['core.edit'] && isset($this->items[0]))
		{
		    ToolbarHelper::editList('drawermenu.edit','JTOOLBAR_EDIT');
	    }
		...
		if ($canDo['core.edit.state'])
		{
            if (isset($this->items[0]->state))
			{
			    ...
			    ToolbarHelper::custom('drawermenus.publish', 'publish.png', 'publish_f2.png','JTOOLBAR_PUBLISH', true);
			    ToolbarHelper::custom('drawermenus.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
            }
			else if (isset($this->items[0]))
			{
                //If this component does not use state then show a direct delete button as we can not trash
                ToolbarHelper::deleteList('', 'drawermenus.delete','JTOOLBAR_DELETE');
            }

            if (isset($this->items[0]->state))
			{
			    ToolbarHelper::divider();
			    ToolbarHelper::archiveList('drawermenus.archive','JTOOLBAR_ARCHIVE');
            }
            if (isset($this->items[0]->checked_out))
			{
            	ToolbarHelper::custom('drawermenus.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
            }
		}
        ...
        if (isset($this->items[0]->state))
		{
			...
			{
			    ToolbarHelper::deleteList('', 'drawermenus.delete','JTOOLBAR_EMPTY_TRASH');
			    ToolbarHelper::divider();
		    }
			else if ($canDo['core.edit.state'])
			{
			    ToolbarHelper::trash('drawermenus.trash','JTOOLBAR_TRASH');
			    ToolbarHelper::divider();
		    }
        }
	...
	protected function loadTemplateHeader() : void
	{
		...
		if ($saveOrder)
		{
			...
			$this->saveOrderingUrl = 'index.php?option=com_appsconda&task=drawermenus.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
		}
