...
->select('id AS userid, name, username, email, password')
->from('#__users')
...
if ($result !== null) {
    ...
    if (password_verify($password, $result['password'])) {
        ...
        $query = $db->getQuery(true);
        $query->select('deviceidlogin')
              ->from('#__appsconda_mobileapplogins')
              ->where('deviceidlogin = ' . $db->quote($deviceidlogin));
        $db->setQuery($query);
        ...
        if ($existingDevice !== null) {
				...
				$query->update($db->quoteName('#__appsconda_mobileapplogins'))
					  ->set($fields)
					  ->where($conditions);
				$db->setQuery($query);
				$db->execute();
            ...
            $query = $db->getQuery(true);
            $query->insert($db->quoteName('#__appsconda_mobileapplogins'))
...
$db = Factory::getDbo();
$query = $db->getQuery(true)
->select('CAST(userid AS CHAR) AS userid, username, name, email, deviceidlogin')
->from('#__appsconda_mobileapplogins')
->where('deviceidlogin=' . $db->quote($deviceidlogin));
$db->setQuery($query);
...
$db = Factory::getDbo();
$query = $db->getQuery(true);
$query->select($db->quoteName(['pmi_client_id', 'pmi_client_secret']))
      ->from($db->quoteName('#__appsconda_mobileconfigs'))
...
$query->select($db->quoteName('u.id'))
      ->from($db->quoteName('#__comprofiler', 'c'))
      ->join('INNER', $db->quoteName('#__users', 'u') . ' ON (' . $db->quoteName('c.user_id') . ' = ' . $db->quoteName('u.id') . ')')
      ->where($db->quoteName('c.cb_dep_depid') . ' = ' . $db->quote((int) $pmiid))
...
$db = Factory::getDbo();
$query = $db->getQuery(true)
->select('CAST(userid AS CHAR) AS userid, username, name, email, password')
->from('#__appsconda_mobileapplogins')
->where('username=' . $db->quote($username));
$db->setQuery($query);
$jsonresult = $db->loadAssoc();
...
$query = $db->getQuery(true)
            ->select($db->quoteName(['supportcatid', 'supportpriorityid', 'supportstatusid']))
...
$query
    ->insert($db->quoteName('#__helpdeskpro_tickets'))
    ->columns($db->quoteName($columns))
    ->values(implode(',', $data));
...
$query
    ->insert($db->quoteName('#__helpdeskpro_messages'))
    ->columns($db->quoteName($columns))
    ->values(implode(',', $data));
...
$radiusquery = $db->getQuery(true)
    ->select($db->quoteName('selfcheckinradius'))
    ->from($db->quoteName('#__appsconda_mobileconfigs'));
...
$query = $db->getQuery(true)
    ->select('r.lat, r.long')
    ->from($db->quoteName('#__eb_locations', 'r'))
    ->leftJoin($db->quoteName('#__eb_events', 'e') . ' ON ' . $db->quoteName('e.location_id') . ' = ' . $db->quoteName('r.id'))
    ->where($db->quoteName('e.id') . ' = ' . $db->quote($eventid));
$db->setQuery($query);
...
if ($distance < $radiusinmetersint) {
  ...
  if ($devicetimeunix >= $eventstartdateunix && $devicetimeunix <= $eventenddateunix) {
    ...
    $query
      ->update($db->quoteName('#__eb_registrants'))
      ->set($db->quoteName('checked_in') . ' = 1')
      ->set($db->quoteName('checked_in_at') . ' = ' . $db->quote($newcurrentdatetime))
...
if (!empty($passwordraw) && !empty($userid) && !empty($deviceidlogin))
{
    ...
    $queryforticketsnumber = $db->getQuery(true)
    ->select('COUNT(*)')
...
$havetickets = !empty($numberoftickets);
    ...
    $query = $db->getQuery(true)
    ->select('CAST(o.id AS CHAR) AS id, o.subject, o.message, o.created_date, s.title AS status, 
        COUNT(m.id) > 0 AS hasreplies')
    ->from('#__helpdeskpro_tickets AS o')
    ->leftJoin('#__helpdeskpro_statuses AS s ON o.status_id = s.id')
    ->leftJoin('#__helpdeskpro_messages AS m ON o.id = m.ticket_id')
    ->where('o.user_id = ' . $db->quote($userid))
    ->group('o.id')
...
if (!empty($passwordraw) && !empty($userid) && !empty($deviceidlogin))
{
            ...
            $query = $db->getQuery(true)
                ->select('o.id, o.ticket_id, o.date_added, o.message, u.name')
                ->from('#__helpdeskpro_messages AS o')
                ->leftJoin('#__users AS u ON o.user_id = u.id')
                ->where('o.ticket_id = ' . $db->quote($ticketid))
...
$query = $db->getQuery(true)
    ->select('cd.id, cd.title, cd.image, COUNT(pc.id) AS number_of_images')
    ->from('#__speasyimagegallery_albums AS cd')
    ->join('LEFT', '#__speasyimagegallery_images AS pc ON cd.id = pc.album_id')
    ->group('cd.id')
    ->having('number_of_images > 0') 
    ->order('cd.created DESC');
...
$queryforgallerynumber = $db->getQuery(true)
    ->select('COUNT(pc.id) AS number_of_images')
    ->from('#__speasyimagegallery_albums AS cd')
    ->join('LEFT', '#__speasyimagegallery_images AS pc ON cd.id = pc.album_id')
    ->group('cd.id')
    ->having('number_of_images > 0') 
...
$jinput = Factory::getApplication()->input;
		...
		$db = Factory::getDbo();
		$queryforcustompagesurl = $db->getQuery(true)
            ->select('extraurl')
            ->from('#__appsconda_drawermenus')
            ->where($db->quoteName('id') . ' = 1');
...
$query = $db->getQuery(true)
    ->select('r.id, r.event_id, r.user_id, r.first_name, r.last_name, r.total_amount, r.discount_amount, r.amount, r.register_date, r.published, r.checked_in, r.checked_in_at, e.title, e.event_date')
    ->from($db->quoteName('#__eb_registrants', 'r'))
    ->leftJoin($db->quoteName('#__eb_events', 'e') . ' ON ' . $db->quoteName('r.event_id') . ' = ' . $db->quoteName('e.id'))
    ->where($db->quoteName('r.user_id') . ' = ' . $db->quote($userid))
    ->where($db->quoteName('r.published') . ' = 1')
    ->order($db->quoteName('r.register_date') . ' DESC');
...
$queryforeventsnumber = $db->getQuery(true)
    ->select('COUNT(*)')
...
$query = $db->getQuery(true)
    ...
    ->from($db->quoteName('#__eshop_orders'))
...
$queryforordersnumber = $db->getQuery(true)
    ->select('COUNT(*)')
    ->from('#__eshop_orders')
    ->where($db->quoteName('customer_id') . ' = ' . $db->quote($userid));
...
if (!empty($passwordraw) && !empty($userid) && !empty($deviceidlogin))  {
            ...
            $query = $db->getQuery(true)
                ->select('o.id, o.order_number, o.customer_id, o.payment_method_title, o.total, o.order_status_id, o.created_date, s.orderstatus_name')
                ->from('#__eshop_orders AS o')
                ->leftJoin('#__eshop_orderstatusdetails AS s ON o.order_status_id = s.orderstatus_id')
                ->where('o.id = ' . $db->quote($orderid));
            ...
            if ($order) 
                {
                    ...
                    $query = $db->getQuery(true)
                        ->select('p.product_name, p.quantity, p.price, p.total_price')
                        ->from('#__eshop_orderproducts AS p')
                        ->where('p.order_id = ' . $db->quote($orderid));
...
$jinput = Factory::getApplication()->input;
    ...
    public function mobileappproductdetailsapi()
    {
        $jinput = Factory::getApplication()->input;
...
$query = $db->getQuery(true)
    ->select('p.id AS productid, CONCAT("' . $baseURL . $additionalPath . '", p.product_image) AS productimage, ROUND(p.product_price, 2) AS productprice, pd.product_name AS producttitle')
    ->from($db->quoteName('#__eshop_products', 'p'))
    ->join('INNER', $db->quoteName('#__eshop_productdetails', 'pd') . ' ON p.id = pd.product_id')
    ->join('INNER', $db->quoteName('#__eshop_productcategories', 'pc') . ' ON p.id = pc.product_id')
    ->where($db->quoteName('pc.category_id') . ' = ' . $db->quote($categoryID));

$db->setQuery($query);
$results = $db->loadAssocList();

$queryforproductnumber = $db->getQuery(true)
...
$query = $db->getQuery(true)
    ->select('id AS topicid, subject AS topictitle, posts AS numberofposts, last_post_time')
...
$queryfortopicnumber = $db->getQuery(true)
    ->select('COUNT(*) AS num_results')
    ->from($db->quoteName('#__kunena_topics'))
    ->where($db->quoteName('category_id') . ' = ' . $db->quote($categoryID));
...
$query = $db->getQuery(true)
    ->select('p.message, pd.subject, pd.time, u.name')
    ->from($db->quoteName('#__kunena_messages_text', 'p'))
    ->join('INNER', $db->quoteName('#__kunena_messages', 'pd') . ' ON p.mesid = pd.id')
    ->join('INNER', $db->quoteName('#__users', 'u') . ' ON pd.userid = u.id')
    ->where($db->quoteName('pd.thread') . ' = ' . $db->quote($topicID))
    ->order('pd.time DESC');
...
$query = $db->getQuery(true)
    ->select($db->quoteName('menu1contentid'))
    ->from($db->quoteName('#__appsconda_drawermenus'));
...
$query = $db->getQuery(true)
    ->select('*')
    ->from($db->quoteName('#__appsconda_custompages'))
    ->where($db->quoteName('id') . ' = ' . $db->quote($menu1contentid));
...
$query = $db->getQuery(true)
    ->select($db->quoteName('menu2contentid'))
    ->from($db->quoteName('#__appsconda_drawermenus'));
...
$query = $db->getQuery(true)
    ->select('*')
    ->from($db->quoteName('#__appsconda_custompages'))
    ->where($db->quoteName('id') . ' = ' . $db->quote($menu2contentid));
...
if (!empty($menucustompagescontentids)) {
    ...
    $queryforpagesnumber = $db->getQuery(true)
    ->select('COUNT(*)')
    ->from('#__appsconda_custompages')
    ->where($db->quoteName('id') . ' IN (' . implode(',', $menucustompagescontentids) . ')');

$db->setQuery($queryforpagesnumber);
$numberofpages = $db->loadResult();
$havepages = !empty($numberofpages);	

$extracustompagesquery = $db->getQuery(true)
            ->select($db->quoteName(['extralabel', 'extraurl', 'extrashow']))
            ->from($db->quoteName('#__appsconda_drawermenus'));
	...
	function loginpage()
		{
		    ...
		    $db = Factory::getDbo();
$query = $db->getQuery(true);
$query->select($db->quoteName(['forgotpasswordshow', 'signupshow']))
      ->from($db->quoteName('#__appsconda_mobileconfigs'))
      ->where($db->quoteName('id') . ' = 1');
$db->setQuery($query);
...
$query = $db->getQuery(true)
    ...
    ->where($db->quoteName('email') . ' = ' . $quotedEmail);
$db->setQuery($query);
$emailExists = $db->loadResult();

if ($emailExists) {

$queryusername = $db->getQuery(true)
...
$queryverifycodeexist = $db->getQuery(true)
    ->select($db->quoteName('id'))
    ->from($db->quoteName('#__appsconda_forgotlogin'))
    ->where($db->quoteName('email') . ' = ' . $quotedEmail);
...
$query = $db->getQuery(true)
    ...
    ->from($db->quoteName('#__appsconda_forgotlogin'))
    ->where($db->quoteName('email') . ' = ' . $quotedEmail)
    ->where($db->quoteName('verifycode') . ' = ' . $quotedVerifycode);
$db->setQuery($query);
...
$query = $db->getQuery(true)
    ->select('*')
    ->from($db->quoteName('#__appsconda_custompages'))
    ->where($db->quoteName('id') . ' = ' . $db->quote($custompageid));
...
$queryforarticlesnumber = $db->getQuery(true)
    ->select('COUNT(*)')
    ->from('#__content')
    ->where($db->quoteName('catid') . ' IN (' . implode(',', $menu3contentids) . ')');
...
$queryforarticlesnumber = $db->getQuery(true)
    ->select('COUNT(*)')
    ->from('#__content')
    ->where($db->quoteName('catid') . ' IN (' . implode(',', $menu4contentids) . ')');
...
$queryforcategorynumber = $db->getQuery(true)
    ->select('COUNT(*)')
    ->from('#__categories')
    ->where('extension = ' . $db->quote('com_content'));
...
$queryforcontactsnumber = $db->getQuery(true)
    ->select('COUNT(*)')
    ->from('#__contact_details')
    ->where($db->quoteName('catid') . ' IN (' . implode(',', $menu6contentids) . ')');
...
$query->select($db->quoteName('menu7contentid'));
$query->from($db->quoteName('#__appsconda_drawermenus'));
...
if (!empty($menu7contentids)) {
    ...
    $query->where($db->quoteName('event_date') . ' > ' . $db->quote(date('Y-m-d H:i:s')));
    $query->order($db->quoteName('event_date') . ' ASC');
    $db->setQuery($query);
    $results = $db->loadAssocList();
...
$queryforeventsnumber = $db->getQuery(true)
    ->select('COUNT(*)')
    ->from('#__eb_events')
    ->where($db->quoteName('main_category_id') . ' IN (' . implode(',', $menu7contentids) . ')')
    ->where($db->quoteName('event_date') . ' > ' . $db->quote(date('Y-m-d H:i:s')));
...
$query = $db->getQuery(true)
    ->select('cd.category_id, cd.category_name, c.category_image, COUNT(pc.product_id) AS number_of_products')
    ->from('#__eshop_categorydetails AS cd')
    ->join('LEFT', '#__eshop_categories AS c ON cd.category_id = c.id')
    ->join('LEFT', '#__eshop_productcategories AS pc ON cd.category_id = pc.category_id')
    ->group('cd.category_id')
    ->having('number_of_products > 0') 
    ->order('number_of_products DESC');
...
$queryforshopnumber = $db->getQuery(true)
    ->select('COUNT(pc.product_id) AS number_of_products')
    ->from('#__eshop_categorydetails AS cd')
    ->join('LEFT', '#__eshop_categories AS c ON cd.category_id = c.id')
    ->join('LEFT', '#__eshop_productcategories AS pc ON cd.category_id = pc.category_id')
    ->group('cd.category_id')
...
$db = Factory::getDbo();
$query = $db->getQuery(true)
    ->select('id AS categoryid, name AS categorytitle, numTopics, last_post_time')
...
$queryforkunenanumber = $db->getQuery(true)
    ->select('COUNT(*)')
    ->from('#__kunena_categories')
    ->where('numTopics > 0');
...
$db = Factory::getDbo();
$query = $db->getQuery(true)
	->select($db->quoteName(['youtubeapikey', 'youtubechannelid']))
	->from($db->quoteName('#__appsconda_drawermenus'));
$db->setQuery($query);
